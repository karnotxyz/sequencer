...
impl AccountTransaction {
    ...
    pub fn perform_pre_validation_stage<S: State + StateReader>(
        ...
        log::debug!("the charge fee here is: {:?}", self.execution_flags.charge_fee);
        if self.execution_flags.charge_fee {
            self.check_fee_bounds(tx_context)?;

            verify_can_pay_committed_bounds(state, tx_context).map_err(Box::new)?;
        }
        log::debug!("pre validation passes");
        Ok(())
    }
    ...
    fn run_non_revertible<S: StateReader>(
        ...
        if matches!(&self.tx, Transaction::DeployAccount(_)) {
            ...
            log::info!("created the new execution context in the run_non_revertible");
    ...
    fn run_or_revert<S: StateReader>(
        ...
        log::debug!("inside the run_or_revert");
        if self.is_non_revertible(&tx_context.tx_info) {
            log::info!("now running the non_revertible");
...
impl<U: UpdatableState> ExecutableTransaction<U> for AccountTransaction {
    fn execute_raw(
        ...
        log::debug!("initial gas here is: {:?}", initial_gas);
        ...
        log::debug!("validate execute call info passed");
        ...
        log::debug!("fee transfer call info passed");
...
impl ValidatableTransaction for AccountTransaction {
    fn validate_tx(
        ...
        log::debug!("starting the validation");
        if !self.execution_flags.validate {
            return Ok(None);
        }
        log::debug!("now checking the fees");
        ...
        log::debug!("got the validation gas");
        ...
        log::debug!("context for the validate_tx has been created!");
        ...
        log::debug!("validate call created");
        ...
        log::debug!("validate call info done");
        ...
        log::debug!("post if condition in the validate_tx");
